...
static const az_span test_component_name = AZ_SPAN_LITERAL_FROM_STR("component_one");
...
static void test_az_iot_pnp_client_twin_parse_received_topic_NULL_rec_topic_fails()
{
  ...
  ASSERT_PRECONDITION_CHECKED(
      az_iot_pnp_client_twin_parse_received_topic(&client, AZ_SPAN_NULL, &response));
}
...
static void test_az_iot_pnp_client_twin_property_begin_component_NULL_client_fails()
{
  az_json_writer jw;
  ASSERT_PRECONDITION_CHECKED(
      az_iot_pnp_client_twin_property_begin_component(NULL, &jw, test_component_name));
}
...
static void test_az_iot_pnp_client_twin_property_begin_component_NULL_jw_fails()
{
  ...
  ASSERT_PRECONDITION_CHECKED(
      az_iot_pnp_client_twin_property_begin_component(&client, NULL, test_component_name));
}
...
static void test_az_iot_pnp_client_twin_property_begin_component_NULL_component_name_fails()
{
  ...
  az_json_writer jw;
  ASSERT_PRECONDITION_CHECKED(
      az_iot_pnp_client_twin_property_begin_component(&client, &jw, AZ_SPAN_NULL));
}
...
static void test_az_iot_pnp_client_twin_property_end_component_NULL_client_fails()
{
  az_json_writer jw;
  ASSERT_PRECONDITION_CHECKED(
      az_iot_pnp_client_twin_property_end_component(NULL, &jw, test_component_name));
}
...
static void test_az_iot_pnp_client_twin_property_end_component_NULL_jw_fails()
{
  ...
  ASSERT_PRECONDITION_CHECKED(
      az_iot_pnp_client_twin_property_end_component(&client, NULL, test_component_name));
}
...
static void test_az_iot_pnp_client_twin_property_begin_component_succeed()
{
  ...
  assert_int_equal(az_iot_pnp_client_twin_property_begin_component(&client, &jw, test_component_name), AZ_OK);
...
static void test_az_iot_pnp_client_twin_property_end_component_succeed()
{
  ...
  assert_int_equal(az_iot_pnp_client_twin_property_begin_component(&client, &jw, test_component_name), AZ_OK);
  assert_int_equal(az_iot_pnp_client_twin_property_end_component(&client, &jw, test_component_name), AZ_OK);
  assert_string_equal(json_buffer, "{\"component_one\":{\"__t\":\"c\"}");
}
...
int test_az_iot_pnp_client_twin()
{
  ...
  const struct CMUnitTest tests[] = {
    ...
    cmocka_unit_test(
        test_az_iot_pnp_client_twin_property_end_component_NULL_component_name_fails),
#endif // AZ_NO_PRECONDITION_CHECKING
